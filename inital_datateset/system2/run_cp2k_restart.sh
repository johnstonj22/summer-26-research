...
echo "======================================"
echo "Starting restart chunk $CHUNK of $MAX_CHUNKS"
...
if [ "$CP2K_EXIT" -eq 0 ]; then
    ...
    if [ "$CHUNK" -lt "$MAX_CHUNKS" ]; then
        NEXT_CHUNK=$((CHUNK + 1))
        echo "Submitting next restart chunk $NEXT_CHUNK with retry reset to 0."
        sbatch --export=ALL,CHUNK="$NEXT_CHUNK",RETRY=0 run_cp2k_restart.sh
    else
    ...
    if [ "$NEW_RETRY" -le "$MAX_RETRIES" ]; then
        echo "Resubmitting same chunk $CHUNK."
        sbatch --export=ALL,CHUNK="$CHUNK",RETRY="$NEW_RETRY" run_cp2k_restart.sh
    else
        echo "Maximum retries reached for chunk $CHUNK. Stopping chain."
        exit 1
    fi
fi